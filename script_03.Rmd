...
sep_old <- 
  ...
  filter(u_grad > 10) %>%
  group_by(title, name, department, id) %>%
...
```{r}
sep_new <- 
  read_excel("new_data/class_enrollment.xlsx", skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
- stepping back and looking at the broader picture, does this affect my analysis?
- what was wrong with the first weeks of a particular class that many people dropped? 
- do number connect to the thing that we are really interested in? 
- what is the Dean really concerned about and how can we make our code reflect that? 
- why is someone interested in the changes in course enrollment

After we have a model, Temperance suggests we should:
- recognize the number are hard to use to interpret students' mentalities
- understand that there might be external circumstances affecting students' decisions
